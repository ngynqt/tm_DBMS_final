...
// Docker MySQL Master connection
$host = "127.0.0.1";
$port = 3308;  // Docker Master port
$user = "root";
$pass = "rootpassword";  // Docker password
$dbname = "my_store";
...
if ($action === 'perf_test_index_effects') {
    ...
    echo json_encode([
        ...
        'analysis' => 'Queries with INDEX sá»­ dá»¥ng "RANGE" hoáº·c "REF" type vÃ  kiá»ƒm tra Ã­t hÃ ng hÆ¡n Full Table Scan'
    ]);
    exit;
}
...
if ($action === 'perf_test_like') {
    ...
    echo json_encode([
        ...
        'comparisons' => $comparisons,
        'optimization_tips' => '
            â€¢ <code>LIKE "%keyword%"</code> (âŒ Cháº­m nháº¥t) = Full text scan<br>
            â€¢ <code>LIKE "keyword%"</code> (âœ… Tá»‘t hÆ¡n) = CÃ³ thá»ƒ dÃ¹ng INDEX<br>
            â€¢ <code>MATCH...AGAINST</code> (âœ… Tá»‘t nháº¥t) = FULLTEXT INDEX (nhanh nháº¥t)
        '
    ]);
    exit;
}
...
if ($action === 'perf_test_composite') {
    ...
    echo json_encode([
        'success' => true,
        'comparisons' => $comparisons,
        'benefits' => 'Composite INDEX <code>(price, rating_average, review_count)</code> giÃºp tá»‘i Æ°u filter Ä‘a Ä‘iá»u kiá»‡n'
    ]);
    exit;
}
